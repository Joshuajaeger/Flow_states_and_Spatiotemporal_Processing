...
expName = 'Flow_endo_post'  # from the Builder filename that created this script
...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='C:\\Users\\Manfred Mustermann\\OneDrive\\Dokumente\\Science\\Scotts lab\\Flow_states_and_Spatiotemporal_Processing\\Code\\Python\\TOJ_Post_lastrun.py',
...
# Initialize components for Routine "Description_Questionaire"
Description_QuestionaireClock = core.Clock()

text = visual.TextStim(win=win, name='text',
    text='You have completed the first part of the experiment. Now we would like to ask you to evaluate 27 statements about your Sports performance from today. You will be able to answer on a scale from "Strongly disagree" to "Strongly agree". \nPlease click with the mouse on the place on the scale that best fits the statement.\nAgain, you should be responding based on the Sports performance from earlier today. \nPlease press the "space bar" to begin with the questions.  \n',
    font='Arial',
...
Quests = visual.TextStim(win=win, name='Quests',
    text='default text',
    font='Arial',
    pos=(0, 0.9), height=0.8, wrapWidth=None, ori=0, 
    color='Black', colorSpace='rgb', opacity=1, 
    languageStyle='LTR',
    depth=-1.0);
Quest_Ans = visual.RatingScale(win=win, name='Quest_Ans', low=1, 
high=5, 
labels=('Strongly disagree', 'Neither agree\n nor disagree', 'Strongly agree'),
tickMarks=None,
markerStart=None, 
markerColor=None, 
markerExpansion=1, 
...
# ------Prepare to start Routine "Description_Questionaire"-------
t = 0
Description_QuestionaireClock.reset()  # clock
frameN = -1
continueRoutine = True
# update component parameters for each repeat
win.mouseVisible = False
key_start = event.BuilderKeyResponse()
# keep track of which components have finished
Description_QuestionaireComponents = [text, key_start]
for thisComponent in Description_QuestionaireComponents:
    if hasattr(thisComponent, 'status'):
        thisComponent.status = NOT_STARTED

# -------Start Routine "Description_Questionaire"-------
while continueRoutine:
    # get current time
    t = Description_QuestionaireClock.getTime()
    frameN = frameN + 1  # number of completed frames (so 0 is the first frame)
    # update/draw components on each frame
...
# -------Ending Routine "Description_Questionaire"-------
for thisComponent in Description_QuestionaireComponents:
    if hasattr(thisComponent, "setAutoDraw"):
        thisComponent.setAutoDraw(False)
...
thisExp.nextEntry()
# the Routine "Description_Questionaire" was not non-slip safe, so reset the non-slip timer
routineTimer.reset()

# set up handler to look after randomisation of conditions etc
controlQ = data.TrialHandler(nReps=1, method='random', 
    extraInfo=expInfo, originPath=-1,
    trialList=data.importConditions('Questionaire.xlsx', selection='0'),
    seed=None, name='controlQ')
thisExp.addLoop(controlQ)  # add the loop to the experiment
thisControlQ = controlQ.trialList[0]  # so we can initialise stimuli with some values
# abbreviate parameter names if possible (e.g. rgb = thisControlQ.rgb)
if thisControlQ != None:
    for paramName in thisControlQ:
        exec('{} = thisControlQ[paramName]'.format(paramName))

for thisControlQ in controlQ:
    currentLoop = controlQ
    # abbreviate parameter names if possible (e.g. rgb = thisControlQ.rgb)
    if thisControlQ != None:
        for paramName in thisControlQ:
            exec('{} = thisControlQ[paramName]'.format(paramName))
    ...
    Quests.setText(Questions)
    Quest_Ans.reset()
    # keep track of which components have finished
    QuestionaireComponents = [Quests, Quest_Ans]
    ...
    while continueRoutine:
        ...
        # *Quests* updates
        if t >= 0.0 and Quests.status == NOT_STARTED:
            # keep track of start time/frame for later
            Quests.tStart = t
            Quests.frameNStart = frameN  # exact frame index
            Quests.setAutoDraw(True)
        # *Quest_Ans* updates
        if t >= 0.0 and Quest_Ans.status == NOT_STARTED:
            # keep track of start time/frame for later
            Quest_Ans.tStart = t
            Quest_Ans.frameNStart = frameN  # exact frame index
            Quest_Ans.setAutoDraw(True)
        continueRoutine &= Quest_Ans.noResponse  # a response ends the trial
    ...
    # store data for controlQ (TrialHandler)
    controlQ.addData('Quest_Ans.response', Quest_Ans.getRating())
    controlQ.addData('Quest_Ans.rt', Quest_Ans.getRT())
    # the Routine "Questionaire" was not non-slip safe, so reset the non-slip timer
    routineTimer.reset()
    thisExp.nextEntry()
    
# completed 1 repeats of 'controlQ'


# set up handler to look after randomisation of conditions etc
Qs = data.TrialHandler(nReps=1, method='random', 
    extraInfo=expInfo, originPath=-1,
    trialList=data.importConditions('Questionaire.xlsx', selection='1:29'),
    seed=None, name='Qs')
thisExp.addLoop(Qs)  # add the loop to the experiment
thisQ = Qs.trialList[0]  # so we can initialise stimuli with some values
# abbreviate parameter names if possible (e.g. rgb = thisQ.rgb)
if thisQ != None:
    for paramName in thisQ:
        exec('{} = thisQ[paramName]'.format(paramName))
...
for thisQ in Qs:
    ...
    # store data for Qs (TrialHandler)
    Qs.addData('Quest_Ans.response', Quest_Ans.getRating())
    Qs.addData('Quest_Ans.rt', Quest_Ans.getRT())
    # the Routine "Questionaire" was not non-slip safe, so reset the non-slip timer
    routineTimer.reset()
    thisExp.nextEntry()
    
# completed 1 repeats of 'Qs'
